...
class DeleteUserAvatar(Command[None]):
    user_id: UUID


class DeleteUserAvatarHandler(CommandHandler[DeleteUserAvatar, None]):
    def __init__(
        self,
    ...
    async def __call__(self, command: DeleteUserAvatar) -> None:
        user_id = UserId(command.user_id)

        await self._user_service.delete_user_avatar(user_id)
        await self._mediator.publish(self._user_service.pull_events())
        await self._uow.commit()

        logger.info("User avatar deleted", extra={"user_id": user_id.to_raw()})
