...
def convert_order_owner_entity_to_dto(order_owner: OrderOwner) -> OrderOwnerDTO:
    return OrderOwnerDTO(
        ...
        orders=[OrderDTO(order_id=order.id, owner_id=order.owner_id) for order in order_owner.orders],
    )
...
def test_convert_entity_to_dto():
    retort = Retort(recipe=(Converter(User, UserDTO, convert_user_entity_to_dto),))
...
def test_convert_sequence_entities_to_list_dtos():
    ...
    assert retort.load([User(1, "Jon"), Order(1, 100)], list[DTOs]) == [
        UserDTO("1", "Jon"),
        OrderDTO(1, 100),
    ]
    assert retort.load((User(1, "Jon"), Order(1, 100)), list[DTOs]) == [
...
def test_convert_sequence_entities_to_tuple_dtos():
    ...
    assert retort.load((User(1, "Jon"), Order(1, 100)), tuple[UserDTO, OrderDTO]) == (
        UserDTO("1", "Jon"),
        OrderDTO(1, 100),
    )
    assert retort.load((User(1, "Jon"), Order(1, 100)), tuple[DTOs, DTOs]) == (
        UserDTO("1", "Jon"),
        OrderDTO(1, 100),
    )
    assert retort.load((User(1, "Jon"), Order(1, 100)), tuple[DTOs, ...]) == (
        UserDTO("1", "Jon"),
        OrderDTO(1, 100),
    )
